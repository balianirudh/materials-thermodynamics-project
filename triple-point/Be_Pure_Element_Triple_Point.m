...
disp("LOADING DATA...")
disp("")
load("data\Be_JANAF_NIST_Table.mat")
disp("COMPLETE")

%% IMPORTED DATA

% Beryllium thermodynamic table data imported from NIST-JANAF (Ni ref)
%   https://janaf.nist.gov/tables/Be-001.html
...
%   T_alpha - alpha-to-beta solid-state phase transformation temperature [K]
%   Tm - melting temperature [K]
%   Tg - vaporization temperature [K]
%   index_T_alpha - index of T_alpha in T going from alpha-to-beta solid phase
...
options_cf1 = fitoptions('Method', 'NonlinearLeastSquares', ...
                     'StartPoint', [0, 20, -5, 2], ...
                     'Lower', [0, -Inf, -Inf, -Inf], ...
                     'Upper', [0, Inf, Inf, Inf], ...
                     'Robust', 'Bisquare');

options_cf2 = fitoptions('Method', 'NonlinearLeastSquares', ...
                     'StartPoint', [5, 20, -5, 2, -0.5], ...
                     'Lower', [-Inf, -Inf, -Inf, -Inf, -Inf], ...
                     'Upper', [Inf, Inf, Inf, Inf, Inf], ...
                     'Robust', 'Bisquare');

% Curve Fit 1: T = [0, T_alpha), Alpha-Be
T1 = T(7:index_T_alpha); % temperature range before first phase transformation [K]
Cp1 = Cp(7:index_T_alpha); % specific heat for temperature range before first phase transformation [J/(K*mol)]
[fit1, gof1] = fit(T1, Cp1, shomate_cf1, options_cf1); % curve fit 1 [curve fitted polynomial coefficients, goodness of fit parameters]
...
c1 = fit1_coeffs(3);
d1 = fit1_coeffs(4);
T1 = linspace(0, T1(end), T1(end));

Cp_cf1 = a1 + b1.*T1 + c1.*T1.^2 + d1.*T1.^3; % Cp curve fit 1 (alpha-Be) [J(K*mol)]

% Curve Fit 2: T = (T_alpha, Tm), Beta-Be
T2 = T(index_T_alpha+1:index_T_m_solid); % temperature range before solid-to-liquid transformation [K]
Cp2 = Cp(index_T_alpha+1:index_T_m_solid); % specific heat for temperature range before solid-to-liquid transformation [J(K*mol)]

a2 = Cp2(1); % from the values of Cp for beta-Be, it is easy to conclude that specific heat is a horizontal line
b2 = 0;
T2 = linspace(T(index_T_alpha), T2(end), T2(end)-T(index_T_alpha)); % extending range to include T_alpha after curve fitting to eliminate the discontinuity

Cp_cf2 = a2 + b2.*T2; % Cp curve fit 2 (beta-Be) [J/(K*mol)]

% Curve Fit 3: T = [Tm, Tg), Liquid
...
L_beta = 6.849; % latent heat of fusion for alpha-Be to beta-Be phase transformation [kJ/mol]
L_f = 7.895; % latent heat of fusion for Be [kJ/mol]
L_v = 291.572; % latent heat of vaporization for Be [kJ/mol]

% Piecewise specific heat function (defined symbolically)
Cp_fun1 = @(T) a1 + (b1*T) + (c1*T.^2) + (d1*T.^3); % Cp from T = [0, T_alpha), [J/(K*mol)]
Cp_fun2 = @(T) a2 + (b2*T);  % Cp from T = [T_alpha, Tm), [J/(K*mol)]
...
% Piecewise specific heat function (defined symbolically)
S_integrand1 = @(T) (a1 + (b1*T) + (c1*T.^2) + (d1*T.^3))./T; % S integrand from T = [0, T_alpha), [J/(K^2*mol)]
S_integrand2 = @(T) (a2 + (b2*T))./T;  % S integrand from T = [T_alpha, Tm), [J/(K^2*mol)]
...
V_s = 4.85*(10^-6); % molar volume of solid Be [m^3/mol]
V_l = 4.85*(10^-6); % molar volume of liquid Be [m^3/mol] 
...
% Bounds
lb = [1500, 0.001]; % [T_min, P_min]
ub = [1900, 100]; % [T_max, P_max]
...
xline(T_alpha, Label="T_{\alpha}", LineStyle="--", LabelHorizontalAlignment='left')
xline(Tm, Label="T_m", LineStyle="--")
xline(Tg, Label="T_v", LineStyle="--")
ylim([0 45])
legend("NIST-JANAF", "\alpha-Be", "\beta-Be", "Liquid", "Vapor", "", "", "")
...
% Gibbs Free Energy Solid, Liquid, Gas vs Temperature (@ 1 atm)
figure("Color", "white")

subplot(1, 2, 1)
plot(T_extended, G_solid(T_extended, p_atm), T_extended, G_liquid(T_extended, p_atm), T_extended, G_gas(T_extended, p_atm), 'LineWidth', 1.5)

xlabel("T [K]", "FontWeight", "bold", "FontSize", 11)
ylabel("G" + char(176) + " [J mol^{-1}]", "FontWeight", "bold", "FontSize", 11)
xline(T_alpha, Label="T_{\alpha}", LineStyle="--", LabelHorizontalAlignment='left')
xline(Tm, Label="T_m", LineStyle="--")
xline(Tg, Label="T_v", LineStyle="--")
ylim([-4*(10^5), 4*(10^5)])
legend("G_{solid}", "G_{liquid}", "G_{vapor}")
fontname("Times New Roman")

% Gibbs Free Energy Solid, Liquid, Gas vs Temperature (@ P_triple)

subplot(1, 2, 2)
plot(T_extended, G_solid(T_extended, P_triple), T_extended, G_liquid(T_extended, P_triple), T_extended, G_gas(T_extended, P_triple), 'LineWidth', 1.5)

xlabel("T [K]", "FontWeight", "bold", "FontSize", 11)
ylabel("G" + char(176) + " [J mol^{-1}]", "FontWeight", "bold", "FontSize", 11)
xline(T_triple, Label="T_{triple}", LineStyle="--")
ylim([-4*(10^5), 4*(10^5)])
legend("G_{solid}", "G_{liquid}", "G_{vapor}")
fontname("Times New Roman")